...
const watch = ({ products }) => {
  ...
  return (
...

      {
        filteredProducts.length === 0 ? (
          <div className={styles.noProductFound}>
            ...
            <p>We're sorry, but there are no products available for the selected color.</p>
    ...
    <div className={styles.mobileCategoriesContainer}>
      {showFilter &&
        <div className={`${styles.filterParentContainer} ${showFilter ? styles.show : ''}`}>
...
export async function getStaticProps() {
  const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/category/audioVideo`);
